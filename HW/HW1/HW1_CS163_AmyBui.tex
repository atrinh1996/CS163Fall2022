    ...
    \end{enumerate}
        ...
        \cbstart
            ...
            \begin{enumerate}[label=\arabic*.]
                ...
                % \item $i\leftarrow \frac{\texttt{left} + \texttt{right}}{2}$




                % \item \textbf{for} $i\leftarrow 0$ to $|R| - 1$
                % \item \hspace{1cm} \textbf{if} $x_{R[i]} < x_q$ \textbf{and} $x_q < x_{R[i+1]}$ \textbf{and} $i < \texttt{max}$ \textbf{and} $b_q > b$ \textbf{then} 
                % \item \hspace{2cm} $j\leftarrow R[i]$; $k\leftarrow R[i+1]$
                % \item \hspace{2cm} \texttt{dir} $\leftarrow$ \textsc{Determinant}($k, q, j$)
                % \item \hspace{2cm} \textbf{if} \texttt{dir} $> 0$ \textbf{then return} \texttt{exterior} \hspace{0.25cm} (\emph{above L, and ccw with neighbors})
                % \item \hspace{2cm} \textbf{else return} \texttt{interior} \hspace{2cm} (\emph{above L, and cw with neighbors})
                % \item \hspace{1cm} \textbf{if} $x_{R[i]} > x_q$ \textbf{and} $x_q > x_{R[i+1]}$ \textbf{and} $i >= \texttt{max}$ \textbf{and} $b_q < b$ \textbf{then}
                % \item \hspace{2cm} $j\leftarrow R[i+1]$; $k\leftarrow R[i]$
                % \item \hspace{2cm} \texttt{dir} $\leftarrow$ \textsc{Determinant}($k, q, j$)
                % \item \hspace{2cm} \textbf{if} \texttt{dir} $> 0$ \textbf{then return} \texttt{interior} \hspace{0.25cm} (\emph{below L, and ccw with neighbors})
                % \item \hspace{2cm} \textbf{else return} \texttt{exterior} \hspace{2cm} (\emph{below L, and cw with neighbors})
            \end{enumerate}
        \cbend
        ...
        \cbstart
            ...
            \begin{enumerate}[label=\arabic*.]
                ...
                % \item \textbf{for} $i \leftarrow 1$ to $n-2$
                % \item \hspace{1cm} $m_i \leftarrow$ slope of line segment $\overline{p_0 p_{i}}$
                % \item \hspace{1cm} $m_{i+1} \leftarrow$ slope of line segment $\overline{p_0 p_{i+1}}$
                % \item \hspace{1cm} \textbf{if} $m_{i} < m_{q}$ and $m_{q} < m_{i+1}$ \textbf{then}
                % \item \hspace{2cm} $j\leftarrow P[i]$
                
                %     \hspace{2cm} $k\leftarrow P[i+1]$
...
\bibitem[3]{officehours}Jake and Diane's office hours, classmates: Stephanie, Sydney, Anju, and MacKenzie with homework problem discussions.
\end{thebibliography}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\end{document}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
