...
%%%%%%%%%% begin text after this line %%%%%%%%%%%%%%
    ...
    \begin{enumerate}[label=\alph*.] 
        ...
        \item First, we check if the starting points $p$ and $q$ are at least $k$ distance from any point in $S$ by checking point inclusion in the region of the circle given by radius $k$ and point $p$ and point $q$. If either $p$ or $q$ are $< k$ away from a point in $S$, then report no path is possible. Otherwise, continue.
        
        Again, assume general position and that both $\mathcal{CH}(S)$ and $\mathcal{VD}(S)$ are computed (each takes $O(n\log n)$ time), overlayed, and their edges added to the same DCEL. We remove the partial edges $\mathcal{VD}(S)$ that are outside $\mathcal{CH}(S)$, since the path must be contained within the hull; we can walk the edges of the $\mathcal{CH}(S)$ to find the intersection points with edges of $\mathcal{VD}(S)$ and remove those beyond the hull. This takes $O(n\log n)$ time. 
    ...
    Topics that I'd like to do the project on:
    \begin{itemize}
        \item Fortune's algorithm for computing the Voronoi Diagram, storing it in a DCEL. 
            \begin{itemize}
                \item It is well described in the textbook.
                \item A sweeping program can look visually active if it runs live and a decreased rate. 
            \end{itemize}
        \item Computing the Delaunay triangulation and getting the Gabriel graph from it, from this homework. 
            \begin{itemize}
                \item triangulation is described in the textbook 
                \item from it, I can use  a subgraph finding algorithm to find the gabriel graph.
            \end{itemize}
        \item Computing path from $p$ to $q$ that is within $k$ distance from a point in a given set and within the convex hull given by the set (the piano movers-like problem from this homework).
            \begin{itemize}
                \item I will use either the elimination method of finding a path described in the homework, 
                \item or find a way for the points to ``walk'' to each other with the given limitations.
            \end{itemize}
    \end{itemize}

    Programming Plan:
    \begin{itemize}
        \item I plan to program the project in C and use the CGAL library for visualization.
        \item A backup in case programming a solution in C takes too long will be to program it in python. 
    \end{itemize}
